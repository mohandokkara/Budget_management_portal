...
if(isset($_POST['n6'])){
	if(!empty($_POST['n1']) || !empty($_POST['n2']) || !empty($_POST['n3']) || !empty($_POST['n4']) || !empty($_POST['n5'])){
		...
		$n4=$_POST['n4'];
		$n5=$_POST['n5'];
		$query =$conn->prepare("insert into rd2 (rd,addon,internalrd,total,disc) values(?,?,?,?,?)");
	...
	if($query){
		$ins =$conn->prepare("update tab1 set a6 =(?) where id=1 ");
		$ins->bind_param("i",$n4);
		$ins->execute();
		if($ins){
	...
	$ins1=$conn->prepare("update tab5 set d5 =(?) where id=1 ");
