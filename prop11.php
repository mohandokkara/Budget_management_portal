...
if(isset($_POST['n7'])){
	if(!empty($_POST['n1']) && !empty($_POST['n2']) && !empty($_POST['n3'])&& !empty($_POST['n4'])&& !empty($_POST['n5'])&& !empty($_POST['n6'])){
		...
		$n5=$_POST['n5'];
		$n6=$_POST['n6'];
		$query =$conn->prepare("insert into pcl (pc1,journals,books,others,total,disc10) values(?,?,?,?,?,?)");
	...
	if($query){
		$ins =$conn->prepare("update tab set t11 =(?) where id=19 ");
		$ins->bind_param("i",$n5);
		$ins->execute();
		if($ins){
	...
	$ins1=$conn->prepare("update tab4 set disc10 =(?) where id=1 ");
