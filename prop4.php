...
if(isset($_POST['n10'])){
	if(!empty($_POST['n1']) && !empty($_POST['n2']) && !empty($_POST['n3'])&& !empty($_POST['n4'])&& !empty($_POST['n5'])&& !empty($_POST['n6'])&& !empty($_POST['n7'])&& !empty($_POST['n8'])&& !empty($_POST['n9'])){
		...
		$n8=$_POST['n8'];
		$n9=$_POST['n9'];
		$query =$conn->prepare("insert into maintainance (labcon,repair,opcost,stationary,mcost,printerco,deptcost,total,disc3) values(?,?,?,?,?,?,?,?,?)");
	...
	if($query){
		$ins =$conn->prepare("update tab set t4 =(?) where id=19 ");
		$ins->bind_param("i",$n8);
		$ins->execute();
		if($ins){
	...
	$ins1=$conn->prepare("update tab4 set disc3 =(?) where id=1 ");
